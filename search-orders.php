...
// منطق البحث
if ((!empty($query) || !empty($date_from) || !empty($date_to)) && isset($orders)) {
...
<head>
    ...
    <title>Search Orders · Teddy Shop</title>
    <!-- Font Awesome -->
...
<div class="admin-wrapper">
    ...
    <main class="admin-main">
        ...
        <div class="search-section">
            ...
            <form action="search-orders.php" method="GET" id="searchForm">
                <div class="search-row">
                    <div class="search-input-group">
                        ...
                        <input type="text"
                               ...
                               placeholder="Order number, customer name, email, status...">
                    </div>
                    ...
                    <a href="search-orders.php" class="search-btn reset-btn">
        ...
        <?php if (!empty($query) || !empty($date_from) || !empty($date_to)): ?>
            ...
            <div class="results-header">
                ...
                <a href="orders.php" class="filter-chip" style="padding: 8px 20px; background: var(--lavender); border-radius: 50px; color: #000; text-decoration: none;">
            ...
            <?php if ($totalResults > 0): ?>
                ...
                <div class="table-container">
                    <table class="orders-table">
                        ...
                        <tbody>
                        <?php foreach($paginatedResults as $id => $order): ?>
                        <tr>
                            <td><strong style="color: var(--primary);"><?= $order['order_number'] ?></strong>
                            <td>
                                <div class="customer-info">
                                    <div class="customer-avatar"><?= strtoupper(substr($order['customer'], 0, 1)) ?></div>
                                    <div>
                            ...
                            <td><?= date('M d, Y', strtotime($order['date'])) ?>
                            <td style="text-align: center;">
                                    <span style="background: var(--lavender); padding: 4px 12px; border-radius: 30px; font-size: 12px;">
                                        <?= $order['items_count'] ?> items
                                    </span>

                            <td><span class="amount">$<?= number_format($order['total'], 2) ?></span>
                            <td><span class="status-badge status-<?= $order['status'] ?>"><?= ucfirst($order['status']) ?></span>
                            <td><span style="font-size: 13px;"><?= $order['payment_method'] ?></span>
                            <td>
                                <div class="action-buttons">
                                    <button class="action-btn view" onclick="viewOrder(<?= $id ?>)" title="View">
                                        <i class="fa-solid fa-eye"></i>
                                    </button>
                                    <button class="action-btn edit" onclick="editOrder(<?= $id ?>)" title="Edit">
                                        <i class="fa-solid fa-pen"></i>
                                    </button>
                                    <button class="action-btn delete" onclick="deleteOrder(<?= $id ?>)" title="Delete">
